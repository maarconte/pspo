...
export const authService = {
	...
	sendMagicLink: async (email: string): Promise<void> => {
		...
		try {
			...
			throw new Error(error.code ? getAuthErrorMessage(error.code) : 'Failed to send sign-in link');
		}
	},
	...
	completeMagicLinkSignIn: async (email?: string): Promise<User> => {
		...
		try {
			...
			throw new Error(error.code ? getAuthErrorMessage(error.code) : 'Authentication failed');
		}
	},
