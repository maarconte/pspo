...
export const authService = {
	...
	sendMagicLink: async (email: string): Promise<void> => {
		...
		try {
			...
			throw new Error(error.code ? getAuthErrorMessage(error.code) : 'Échec de l\'envoi du lien de connexion');
		}
	},
	...
	completeMagicLinkSignIn: async (email?: string): Promise<User> => {
		...
		try {
			...
			throw new Error(error.code ? getAuthErrorMessage(error.code) : 'Échec de la connexion');
		}
	},
